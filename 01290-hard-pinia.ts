...
type ObjectValueReturnType<T> = {
  readonly [P in keyof T]: T[P] extends (arg: any[]) => infer R ? R : never
}
declare function defineStore<State, Getters, Actions>(store: {
  id: string
  state: () => State
  getters: Getters & ThisType<Readonly<State> & ObjectValueReturnType<Getters>>
  actions: Actions & ThisType<State & ObjectValueReturnType<Getters> & Actions>
}): State & ObjectValueReturnType<Getters> & Actions
