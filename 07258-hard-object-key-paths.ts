...
// ============= Your Code Here =============
type ObjectKeyPaths<T, U extends keyof T = keyof T> = T extends
  | object
  | unknown[]
  ? U extends keyof T & (string | number)
    ? `${T extends unknown[] ? `[${U}]` | U : U}${
        | ''
        | `${T[U] extends unknown[] ? '' | '.' : '.'}${ObjectKeyPaths<T[U]>}`}`
    : never
  : never

type Test = ObjectKeyPaths<{
  refCount: number
  person: { name: string; age: number }
}>
